use crate::{
    abstract_trait::{
        DynKafka, DynResetTokenCommandRepository, DynResetTokenQueryRepository,
        DynUserCommandRepository, DynUserQueryRepository, PasswordServiceTrait,
    },
    cache::CacheStore,
    domain::{
        requests::{CreateResetPasswordRequest, CreateResetTokenRequest, EmailRequest},
...
impl PasswordServiceTrait for PasswordResetService {
    async fn forgot(&self, email: &str) -> Result<ApiResponse<bool>, ServiceError> {
        ...
        let user = match self.user_query.find_by_email(email).await {
            Ok(Some(u)) => u,
            Ok(None) => {
            ...
            Err(e) => {
                error!("❌ DB error fetching user by email: {:?}", e);
                self.complete_tracing_error(
                    &tracing_ctx,
                    method.clone(),
            ...
            .await
        {
            ...
            self.complete_tracing_error(&tracing_ctx, method.clone(), "Failed to save reset token")
                .await;
            return Err(ServiceError::Repo(e));
        }
        ...
        self.complete_tracing_success(&tracing_ctx, method.clone(), "Reset email sent")
            .await;
    ...
    async fn reset_password(
            ...
            .await
        {
            ...
            Ok(None) => {
                ...
                self.complete_tracing_error(&tracing_ctx, method.clone(), "Reset token not found")
                    .await;
        ...
        self.user_command
            .update_password(reset_token_model.user_id, &data.password)
            .await
            .map_err(|e| {
        ...
        if let Err(e) = self
            .reset_token_command
            .delete_reset_token(reset_token_model.user_id)
            .await
        {
    ...
    async fn verify_code(&self, code: &str) -> Result<ApiResponse<bool>, ServiceError> {
        ...
        let user = match self.user_query.find_verify_code(code).await {
            Ok(Some(user)) => user,
            Ok(None) => {
            ...
            Err(e) => {
                error!("❌ DB error querying verify code: {:?}", e);
                self.complete_tracing_error(
                    &tracing_ctx,
                    method.clone(),
                    "Failed to find verify code",
                )
                .await;
                return Err(ServiceError::Repo(e));
            }
        ...
        if let Err(e) = self.user_command.update_isverifed(user.user_id, true).await {
        ...
        let email_data = EmailTemplateData {
            title: "Verification Success".to_string(),
            message: "Your account has been successfully verified. Click the button below to view or manage your card.".to_string(),
            button: "Go to Dashboard".to_string(),
            link: "https://sanedge.example.com/card/create".to_string(),
        };

        let email_req = EmailRequest {
            to: user.email.clone(),
            subject: "Email Verification Success".to_string(),
            data: email_data,
        };
        ...
        if let Err(e) = self
            ...
            .await
        {
            error!("❌ Failed to publish verification success event: {:?}", e);
            self.complete_tracing_error(
                &tracing_ctx,
                method.clone(),
                "failed to publish verification success event",
            )
            .await;
            return Err(ServiceError::Custom(
        ...
        self.cache_store
            .set_to_cache(&cache_key, &true, Duration::seconds(84000));

        self.complete_tracing_success(&tracing_ctx, method, "Account verified")
            .await;

        info!("✅ Code verified and email sent for user: {}", user.email);
