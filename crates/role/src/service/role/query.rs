use crate::{
    abstract_trait::role::{repository::DynRoleQueryRepository, service::RoleQueryServiceTrait},
    domain::{
    ...
    async fn find_by_name(&self, name: String) -> Result<ApiResponse<RoleResponse>, ServiceError> {
        ...
        let tracing_ctx = self.start_tracing(
            ...
            vec![
                ...
                KeyValue::new("role.name", name.clone()),
            ],
        );
        ...
        let role = match self.query.find_by_name(&name).await {
            Ok(Some(role)) => {
                info!("✅ Found role in DB: {} (Name: {name})", role.role_name);
                self.complete_tracing_success(
                    &tracing_ctx,
                    method.clone(),
                    "Role retrieved from DB",
            ...
            Ok(None) => {
                let msg = format!("❌ Role not found with name: {name}");
                error!("{msg}");
                self.complete_tracing_error(&tracing_ctx, method.clone(), "Role not found")
                    .await;
                return Err(ServiceError::Custom("Role not found".to_string()));
            ...
            Err(e) => {
                let msg = format!("❌ Database error while finding role '{name}': {e:?}");
                error!("{msg}");
                self.complete_tracing_error(&tracing_ctx, method.clone(), "Database error")
                    .await;
                return Err(ServiceError::Repo(e));
        ...
        info!(
            "✅ Role retrieved: '{}' (ID: {})",
            response.data.role_name, response.data.role_id
        );
