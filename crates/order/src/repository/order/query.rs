use crate::{
    ...
    domain::requests::order::FindAllOrder, model::order::Order as OrderModel,
};
...
impl OrderQueryRepositoryTrait for OrderQueryRepository {
    async fn find_all(
        ...
        let rows = sqlx::query!(
            ...
            FROM orders o
            WHERE ($1::TEXT IS NULL OR o.user_id::TEXT ILIKE '%' || $1 || '%')
